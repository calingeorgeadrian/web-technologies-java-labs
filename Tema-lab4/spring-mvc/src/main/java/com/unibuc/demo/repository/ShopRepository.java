...
public class ShopRepository {
    ...
    public Shop updateShop(String cui, Shop shopDto) {
        Shop shop = shopList.stream().filter(sh -> sh.getCUI().equals(cui)).findFirst().get();
        int index = shopList.indexOf(shop);
        shopList.set(index, shopDto);
        return this.getByCui(cui).get();
    }

    private void setUpContextForShopRepository() {
        createAndSave("Profi", "1111", retrieveProductsFromProductRepository(shopOneStartRange, shopOneEndRange));
        createAndSave("Penny", "2222", retrieveProductsFromProductRepository(shopTwoStartRange, shopTwoEndRange));
    }
